...
fureya.ev.on('group-participants.update', async (anu) => {
console.log(anu)
try {
let metadata = await fureya.groupMetadata(anu.id)
let participants = anu.participants
for (let num of participants) {
try {
ppuser = await fureya.profilePictureUrl(num, 'image')
} catch {
ppuser = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
}
try {
ppgroup = await fureya.profilePictureUrl(anu.id, 'image')
} catch {
ppgroup = 'https://i.ibb.co/s2KvYYf/20230524-060103.png'
}
let nameUser = await fureya.getName(num)
let membr = metadata.participants.length
if (anu.action == 'add') {
await fureya.sendMessage(anu.id, { externalAdReply:{
title: `© FauziDevID`,
mediaType: 1,
renderLargerThumbnail: true,
thumbnail: ppuser,
sourceUrl: gc,
}, text:`*Halo @${num.split('@')[0]} Welcome to ${metadata.subject}*`, mentions: [num]}, { quoted: uzii })
} else if (anu.action == 'remove') { 
await fureya.sendMessage(anu.id, { image: { url: ppuser }, caption: `
Selamat tinggal *@${num.split('@')[0]}*`, mentions: [num]}, { quoted: uzii })
}
}
} catch (err) {
console.log(err)
